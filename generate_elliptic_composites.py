...
class SampleConfig:
    ...
    gmin: float = 0.005  # 椭圆间最小间隙
    boundary_margin: float = 0.02  # 椭圆到矩形边界的最小距离
...
def get_ellipse_bounding_radius(a: float, b: float) -> float:
    """
    计算椭圆的外接圆半径（保守估计）
    用于优化中心点采样范围
    """
...
def try_place_one(rect: Polygon,
    ...
    for _ in range(cfg.per_ellipse_max_tries):
        ...
        # 计算椭圆可能的最大外接半径
        max_radius = get_ellipse_bounding_radius(a, b)
        
        # 考虑边界距离限制，调整中心点采样范围
        margin_total = cfg.boundary_margin + max_radius
        
        # 检查是否有足够空间放置椭圆
        if margin_total * 2 >= cfg.Lx or margin_total * 2 >= cfg.Ly:
            continue
            
        # 在收缩后的区域内采样中心点
        x = random.uniform(margin_total, cfg.Lx - margin_total)
        y = random.uniform(margin_total, cfg.Ly - margin_total)

        ell = make_ellipse_polygon(x, y, a, b, theta, cfg.ellipse_resolution)
        
        # 检查椭圆到边界的实际距离
        boundary_distance = rect.exterior.distance(ell)
        if boundary_distance < cfg.boundary_margin:
            continue

        # 检查与其他椭圆的距离
        ok = True
        for other_ell in placed_buffers:
            if ell.distance(other_ell) < cfg.gmin:
                ok = False
                break
...
def generate_sample(cfg: SampleConfig, verbose: bool = True) -> SampleResult:
    ...
    placed_ellipses: List[Polygon] = []  # 存储椭圆本体，用于距离计算
    ...
    if want_by_count:
        while len(placed_params) < int(cfg.N):
            if trials >= cfg.max_trials:
                break
            trials += 1
            ep = try_place_one(rect, placed_ellipses, cfg)
            if ep is None:
                continue
            placed_params.append(ep)
            ell = make_ellipse_polygon(ep.x, ep.y, ep.a, ep.b, ep.theta_deg, cfg.ellipse_resolution)
            placed_ellipses.append(ell)
    ...
    if want_by_phi:
        ...
        while current_phi(len(placed_params), areas_sum) < target:
            ...
            areas_sum += math.pi * ep.a * ep.b
            if verbose and len(placed_params) % 10 == 0:
                phi_current = areas_sum / A_rect
                print(f"Placed {len(placed_params)} ... phi={phi_current:.4f}, trials={trials}")
    ...
    if verbose:
        ...
        print(f"Boundary margin: {cfg.boundary_margin}, Ellipse spacing: {cfg.gmin}")
...
def save_sample_as_json(sample: SampleResult, out_dir: Path) -> Path:
    ...
    data = {
        "meta": {
            ...
            "boundary_margin": sample.config.boundary_margin,  # 添加边界距离记录
    ...
    with open(csv_path, "a", newline="", encoding="utf-8") as f:
        ...
        if write_header:
            writer.writerow([
                ...
                "N", "phi", "gmin", "boundary_margin",  # 添加边界距离列
        ...
        writer.writerow([
            ...
            len(sample.ellipses), sample.phi, cfg.gmin, cfg.boundary_margin,
...
def parse_args():
    ...
    p.add_argument("--gmin", type=float, default=0.005, help="Minimum distance between ellipses.")
    p.add_argument("--boundary_margin", type=float, default=0.02, help="Minimum distance from ellipses to rectangle boundary.")
...
def main():
    ...
    cfg = SampleConfig(
        ...
        gmin=args.gmin,
        boundary_margin=args.boundary_margin,  # 新增边界距离参数
